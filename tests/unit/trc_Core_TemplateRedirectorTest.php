...
class trc_Core_TemplateRedirectorTest extends \PHPUnit_Framework_TestCase {
	...
	public function it_should_be_instantiatable() {
		Test::assertInstanceOf( 'trc_Core_TemplateRedirector', new trc_Core_TemplateRedirector() );
	}
	...
	public function it_should_not_redirect_if_user_can_access_post() {
		...
		$post_types = Test::replace( 'trc_Core_PostTypes' )->method( 'is_restricted_post_type', true )->get();
		$sut->set_post_types( $post_types );

		$user = Test::replace( 'trc_Core_UserInterface' )->method( 'can_access_post', true )->get();
		$sut->set_user( $user );
	...
	public function it_should_redirect_if_user_has_no_access_to_post() {
		...
		$user = Test::replace( 'trc_Core_UserInterface' )->method( 'can_access_post', false )->get();
		$sut->set_user( $user );
